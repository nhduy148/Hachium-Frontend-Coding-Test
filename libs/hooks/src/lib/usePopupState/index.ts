...
export function usePopupState({
  ...
  const setState = useCallback((state: CoreState | ((prevState: CoreState) => CoreState)) => {
  ...
  const toggle = useEvent((eventOrAnchorEl?: SyntheticEvent | Element | null) => {
  ...
  const open = useEvent((eventOrAnchorEl?: SyntheticEvent | Element | null) => {
    ...
    const anchorPosition =
      typeof clientX === 'number' && typeof clientY === 'number' ? { left: clientX, top: clientY } : undefined;
  ...
  const close = useEvent((eventOrAnchorEl?: SyntheticEvent | Element | null) => {
    const event = eventOrAnchorEl instanceof Element ? undefined : eventOrAnchorEl;
  ...
  const setOpen = useCallback((nextOpen: boolean, eventOrAnchorEl?: SyntheticEvent<any> | Element | null) => {
    if (nextOpen) {
  ...
  const _setChildPopupState = useCallback(
    (_childPopupState: PopupState | null | undefined) => setState((state) => ({ ...state, _childPopupState })),
    [],
  );
 ...
 * `PopupState`
 */
export function anchorRef({ setAnchorEl }: PopupState): (el: Element | null | null | undefined) => any {
  return setAnchorEl;
}
...
function controlAriaProps({ isOpen, popupId, variant }: PopupState): ControlAriaProps {
  return {
    ...(variant === 'popover'
      ? {
          'aria-haspopup': true,
 ...
 * `PopupState`
 */
export function bindDoubleClick({ isOpen, open, popupId, variant }: PopupState): {
  'aria-controls'?: string;
  'aria-describedby'?: string;
  'aria-haspopup'?: true;
 ...
 * `PopupState`
 */
export function bindPopper({ isOpen, anchorEl, popupId, onMouseLeave }: PopupState): {
  id?: string;
  anchorEl?: Element | null;
  open: boolean;
  onMouseLeave: (event: MouseEvent) => void;
...
function getPopup(element: Element, { popupId }: PopupState): Element | null | undefined {
  if (!popupId) return null;
  const rootNode: any = typeof element.getRootNode === 'function' ? element.getRootNode() : document;
...
function isAncestor(parent: Element | null | undefined, child: Element | null | undefined): boolean {
